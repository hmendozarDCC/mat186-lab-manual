# Lab 3: Mathematical Notation in Quarto

## Learning Objectives
* Distinguish between **Inline** and **Display** math modes in Quarto.
* Use LaTeX syntax to write essential statistical symbols ($\mu, \sigma, \bar{x}$).
* Format complex equations like Linear Regression for professional lab reports.

---

## Introduction
In Data Science, we often need to document the mathematical models we use. Quarto uses a system called **LaTeX** (pronounced "Lay-tek") to render math. This ensures your formulas look professional and are accessible to screen readers.

---

## Part 1: The Two Math Modes
There are two ways to display math in your Quarto documents depending on where you want the formula to appear.

### 1. Inline Math
Use a single dollar sign `$ ... $` when the math should stay inside a sentence.

* **Code:** `The population mean is represented by $\mu$.`
  
* **Result:** The population mean is represented by $\mu$.

### 2. Display Math
Use double dollar signs `$$...$$` to center the formula on its own line.

* **Code:** `$$\bar{x} = \frac{\sum x_i}{n}$$`
  
* **Result:** $$\bar{x} = \frac{\sum x_i}{n}$$



---

## Part 2: Common Symbols for MAT 186
Copy and paste these codes into your Quarto document to practice:

| Statistical Term | LaTeX Code | Rendered Result |
| :--- | :--- | :--- |
| **Sample Mean** | `$\bar{x}$` | $\bar{x}$ |
| **Standard Deviation** | `$\sigma$` | $\sigma$ |
| **Summation** | `$\sum$` | $\sum$ |
| **Correlation Coefficient** | `$r$` | $r$ |
| **Coefficient of Determination** | `$R^2$` | $R^2$ |

---

## Part 3: Writing Equations
For Linear Regression, we combine several symbols. Try typing this in your Quarto file:

**Code:**
`$$y = \beta_0 + \beta_1 x + \epsilon$$`

**Result:**
$$y = \beta_0 + \beta_1 x + \epsilon$$
---


::: {.callout-important}
## 📝 Lab Task 1: 

1. Open your `MAT186_Labs` project in RStudio.
2. Create a new Quarto document (`.qmd`) titled "Math Practice."
3. **Display Math:** Write the formula for the **Standard Deviation** exactly as shown below:
   `$$s = \sqrt{\frac{\sum (x_i - \bar{x})^2}{n - 1}}$$`
4. **Inline Math:** Write `The population mean is represented by $\mu$.`

5. **Render** the document to HTML and confirm the formula appears correctly.
:::



---

## Troubleshooting Math Errors
* **Spaces:** Do not put a space between the `$` and your math code. 
    * *Bad:* `$ \mu $`
    * *Good:* `$\mu$`
* **Missing Signs:** If your formula looks like plain text, check if you forgot the closing `$`.

---

# Lab 3 Final Submission Instructions

::: {.callout-important}
## 📝 Final Checklist for Lab 3
Before submitting, ensure your LaTeX formulas (the Standard Deviation and Population Mean) are rendering correctly in your HTML report. Your folder must be organized exactly as follows:
:::

### Step 1: Organize Your Folder
By now, your **Files pane** in RStudio should show the following structure:

1. You are inside the `Lab_solutions` folder.
2. You have a folder named `Lab03_YourName` (e.g., `Lab03_HenryMendoza`).
3. **Inside** that `Lab03_YourName` folder, you must have these two files:
    - `Lab3_yourname.qmd` (Your Quarto file containing the LaTeX code).
    - `Lab3_yourname.html` (The rendered report showing the math formulas).

### Step 2: Create the ZIP Archive
Since Lab 3 contains both your source code and your rendered math, you must package them together:

1. Open your computer's **File Explorer** (Windows) or **Finder** (Mac).
2. Navigate to your `MAT186_Labs/Lab_solutions/` directory.
3. **Right-click** on your `Lab03_YourName` folder.
4. Select **Compress** (Mac) or **Send to > Compressed (zipped) folder** (Windows).
5. You will now see a new file named `Lab03_YourName.zip`.

### Step 3: Submit to Brightspace

1. Log into **Brightspace** and navigate to **Assignments > Lab 3 Submission**.
2. Upload your **.zip** file.
3. Click **Submit**.

::: {.callout-tip}
## Troubleshooting LaTeX
If your formulas do not look like the math in the instructions, make sure you have enclosed the Standard Deviation formula in double dollar signs `$$...$$` and the population mean in single dollar signs `$...$`.
:::

### **Accessibility Check**
When you use this LaTeX method, students using screen readers can hear the math described clearly (e.g., "x bar equals fraction sum x sub i over n"). This is much better than using an image of an equation, which a screen reader cannot "see."

---

::: {.callout-tip}
### Done with the Lab?
If you have finished all the tasks above, you can verify your work against the official solutions.

<center>
<a href="../solutions.html" class="btn btn-outline-success" role="button">🔓 Access Lab Solutions</a>
</center>
:::

