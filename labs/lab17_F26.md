...
# 2. Create the plot
# We use lwt (mother's weight) on the x-axis 
# and bwt (birth weight) on the y-axis
ggplot(data, aes(x = lwt, y = bwt)) +
  geom_point(alpha = 0.5, color = "blue") +  # Adds the data points
  geom_smooth(method = "lm", color = "red") + # Adds the linear regression line
  theme_minimal() +
  labs(title = "Scatter Plot of Mother's Weight vs. Birth Weight",
       x = "Mother's Weight (lbs)",
       y = "Birth Weight (grams)")
```       
# Task 3: Correlation
